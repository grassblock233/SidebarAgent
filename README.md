...
一个使用 Vite、React 和 TypeScript 构建的 Chrome / Edge Manifest V3 扩展。它可以将网页选中文字或当前视口内的可见 DOM 文字送入侧边栏，并使用所选 AI 提供商进行解释、总结、翻译和连续追问。
...
- 当前来源和对话保存在 `chrome.storage.session`，浏览器会话结束后清除。
- 旧版 DeepSeek 配置会自动迁移到当前 settings v2 结构，旧存储键继续保留以便回退。
- 选中文字、网页标题、网址、清洗后的视口内容和对话会发送给当前 AI 提供商。
